"""
This file contains tools for visualizing and saving NEAT networks.
"""
...
def dict_to_config(file_path, config_dict):
    with open(file_path, "w") as file:
        # Write a header (optional)
        file.write("# NEAT configuration for the bit-sequence memory experiment.\n\n")
        ...
        for section, params in config_dict.items():
            # Skip the "Simulation" section since it's not part of the config format
            ...
            for key, value in params.items():
                # Convert boolean values to 'True'/'False' as strings
                if isinstance(value, bool):
                    value = str(value).lower()
                # Format floating point numbers with proper precision
                elif isinstance(value, float):
                    value = f"{value:.10g}"  # Keeps precision while avoiding scientific notation if possible
                
                file.write(f"{key.ljust(30)} = {value}\n")  # Align keys for better readability
...
def config_to_dict(**kwargs):
    ...
    for line in config_lines:
        ...
        if line.startswith("[") and line.endswith("]"):
            ...
            neat_dict[current_section] = {}  # Create a new section
            continue

        # Parse key-value pairs
        if "=" in line and current_section:
            key, value = map(str.strip, line.split("=", 1))  # Split on first '='
            ...
            neat_dict[current_section][key] = value  # Store parsed value

    if kwargs:
        neat_dict["Simulation"] = kwargs
...
def draw_net(config, genome, view=False, filename=None, node_names=None, show_disabled=True, prune_unused=False,
    ...
    # Delete the .gv file after the .svg is generated
    os.remove(filename)
